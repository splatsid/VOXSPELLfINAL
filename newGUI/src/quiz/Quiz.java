...
public abstract class Quiz {
	...
	public void popupendoflevel(int numcorrect) {
		...
		gobackmainmenu = new Button("Continue");
		VBox controls = new VBox(10);
		controls.setAlignment(Pos.CENTER);

		if (numcorrect >= 9 || numcorrect >= thetestwords.size() -1) { // MOVIE POP UP
			watchmovie = new Button("Watch Reward Video");
			label = new Label(
					"Congratulations you got " + numcorrect + " correct. \n This means you can watch a video!");
			controls.getChildren().addAll(gobackmainmenu, watchmovie, label);
...
watchmovie.setOnAction(new EventHandler<ActionEvent>() {
	...
	public void handle(ActionEvent arg0) {
		...
		Scene moviescene = new Scene(buttons);
		MovieScene movie = new MovieScene(movieStage, moviescene);
		moviescene = movie.moviescene();
		...
		movieStage.show();
			...
			controls.getChildren().addAll(gobackmainmenu, label);
			bp.setCenter(controls);
			scene = new Scene(bp, 600, 500);
		}
